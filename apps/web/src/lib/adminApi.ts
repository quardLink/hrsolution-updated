...
// was independently building the URL, attaching the x-admin-password header,
// checking res.ok, and parsing an optional {error} body — this collapses
// that boilerplate to one call while preserving each call site's original
...
export async function adminFetch<T>(baseUrl: string, path: string, opts: AdminFetchOptions): Promise<T> {
  const { password, method = "GET", body, errorMessage = "Request failed" } = opts;
